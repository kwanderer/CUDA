...
int main( int argc, char* argv[]) {
	cout << "*** Sobel filtering on GPU ***" << endl 
		 << "***    Solution by Dmitry Odinoky    ***" << endl << endl;
	...
	if (generateNoiseImage) {
		...
		if (computeOnGPU) {
			...
			cudaError_t cudaStatus = sobelFilterGrayscaleCUDA(inputImage, outputImage, dp.maxThreadsPerBlock);
			if (cudaStatus != cudaSuccess) {
				cerr << "CUDA sobel filter failed!";
				return 1;
			}
			...
			sobelFilterGrayscaleCPU(inputImage, outputImage);
		}
	}
